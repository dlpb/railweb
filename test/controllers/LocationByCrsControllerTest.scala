...
import controllers.location.list.crs.LocationsByCrsController
...
class LocationByCrsControllerTest
    extends AnyFlatSpec
    with Matchers
    with MockitoSugar {
  ...
  "Location By Tiploc Controller" should "return a list of locations unfiltered" in {
    ...
    val controller = new LocationsByCrsController(
  ...
  it should "only show stations filtered by ID when the id flag is not equal to all" in {
    ...
    val locations = controller.getListOfLocations(filterId = "KGX")
  ...
  it should "get visit status for each location" in {
    ...
    val visitStatus = controller.getVisitStatus(locationService.sortedListLocationsGroupedByCrs, visitedIds)
  ...
  it should "get all visited locations" in {
    ...
    val visitedLocationsCount = controller.getVisitedLocationIdCount(locationService.sortedListLocationsGroupedByCrs, visitedIds)
