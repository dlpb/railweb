...
class VisitLocationServiceTest
    extends AnyFlatSpec
  ...
  override protected def beforeAll(): Unit = {
    super.beforeAll()
    when(mockLocationService.findFirstLocationByTiploc("LST")).thenReturn(Some(location))
    when(mockLocationService.findFirstLocationByTiploc("KGX")).thenReturn(Some(location2))
  }

  "Location Visit Service" should "get no visits to a location if a user has none" in {
  ...
  it should "work out visited locations for an event" in {
    ...
    visitedLocations.size should be(2)
  }
